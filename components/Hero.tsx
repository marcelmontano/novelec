...
export const Hero: React.FC = () => {
  return (
    <div className="relative overflow-hidden bg-slate-900 pt-32 pb-20 lg:pt-40 lg:pb-32">
      {/* Banner de Advertencia B2B - High Visibility */}
      ...
      <div className="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 flex flex-col items-center text-center">
        <div className="inline-flex items-center rounded-full bg-emerald-500/10 px-5 py-2 text-xs font-black text-emerald-400 ring-1 ring-inset ring-emerald-400/30 mb-10 animate-fade-in-up backdrop-blur-md">
          <Building2 size={14} className="mr-2" /> FIRMA ESPAÑOLA CON 10+ AÑOS EN CUBA
        </div>
        
        <h1 className="text-5xl font-black tracking-tighter text-white sm:text-7xl lg:text-9xl mb-8 leading-[0.9] uppercase">
          Liquidación de <br />
          <span className="gradient-text">2 Contenedores</span>
        </h1>
        
        <p className="max-w-3xl text-xl sm:text-2xl text-slate-300 mb-12 font-medium leading-tight">
          Oportunidad única para <span className="text-white font-black underline decoration-emerald-500">Inversores y Empresas</span>. 
          Venta exclusiva de lotes cerrados EcoFlow y Deye en Almacenes Tradex. No se venden unidades sueltas.
        </p>
        ...
        <div className="bg-slate-950/50 border border-slate-800 p-6 rounded-3xl mb-12 max-w-2xl w-full backdrop-blur-md flex flex-col sm:flex-row items-center justify-center gap-8 shadow-2xl">
            ...
            <div className="text-center sm:text-left">
                <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest block mb-1">Estatus Mercancía</span>
            ...
            <div className="text-center sm:text-left">
                ...
                <span className="text-2xl font-black text-cyan-400 uppercase tracking-tighter">Exterior / USDT</span>
            </div>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 sm:gap-8 w-full border-t border-slate-800 pt-12">
           {[
             { icon: <ShieldCheck className="text-emerald-400" />, title: "VENTA B2B", desc: "Solo Lotes Completos" },
             { icon: <TrendingUp className="text-cyan-400" />, title: "ALTA RENTABILIDAD", desc: "Precios de Fábrica" },
             { icon: <AlertCircle className="text-yellow-400" />, title: "SIN EFECTIVO CUP", desc: "Solo Divisa Exterior" },
             { icon: <Award className="text-purple-400" />, title: "FIRMA DIRECTA", desc: "Garantía de Importador" },
           ].map((item, i) => (
